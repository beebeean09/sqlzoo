...
def song_title_counts
  ...
  execute(<<-SQL)
    SELECT
      song, COUNT(DISTINCT albums.title)
    FROM
      tracks
    JOIN albums
      ON albums.asin = tracks.album
...
def rock_superstars
  ...
  execute(<<-SQL)
    SELECT
      artist, COUNT(DISTINCT albums.title)
    FROM
      albums
    JOIN styles
    ...
    GROUP BY
      albums.artist
    ORDER BY COUNT(DISTINCT albums.title) DESC
    LIMIT 1
  SQL
end
...
def expensive_tastes
  ...
  execute(<<-SQL)
    ...
    JOIN (
      ...
      GROUP BY
        albums.asin ) AS album_track_counts ON styles.album = album_track_counts.asin
    GROUP BY
      styles.style
    ORDER BY
